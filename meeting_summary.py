"""
Meeting Summary Bot - v2
Google Calendar から今週（月〜金）の承諾済み会議を集計し、
Slack の自分宛 DM に投稿する

投稿タイミング: 毎週金曜 21:00 JST（GitHub Actions で実行）
"""
...
MY_EMAIL    = os.environ["MY_EMAIL"]          # 例: shinji.takenaka@lxdesign.me
SLACK_TOKEN = os.environ["SLACK_BOT_TOKEN"]   # xoxb-... Bot User OAuth Token
...
def fetch_events(service, time_min: datetime, time_max: datetime) -> list:
    """ページネーション対応で全イベントを取得"""
...
def get_duration_minutes(ev: dict) -> int:
...
def is_solo(ev: dict) -> bool:
    attendees = ev.get("attendees", [])
    if not attendees:
        return True
    real = [a for a in attendees
            if not a.get("email", "").endswith("@resource.calendar.google.com")]
    return len(real) <= 1


def get_my_status(ev: dict) -> str:
...
def filter_and_categorize(events: list) -> list:
    result = []
    for ev in events:
        ...
        result.append({
            ...
            "minutes":  minutes,
            "date":     ev["start"]["dateTime"][:10],    # YYYY-MM-DD
            "time":     ev["start"]["dateTime"][11:16],  # HH:MM
        })
    return result
...
def make_bar(minutes: int, total: int, width: int = 16) -> str:
    if total == 0:
        return "░" * width
...
# ──────────────────────────────────────────────
# Slack メッセージ構築（Block Kit）
# ──────────────────────────────────────────────

def build_blocks(events: list, week_start: datetime, week_end: datetime) -> list:
    ...
    date_range = (
        f"{week_start.month}/{week_start.day}(月)〜"
        f"{fri.month}/{fri.day}(金)"
    )
    ...
    blocks.append({
        "type": "header",
        "text": {"type": "plain_text", "text": "📅 今週の会議実績レポート", "emoji": True},
    })
    blocks.append({
        "type": "section",
        "text": {"type": "mrkdwn", "text": f"*{date_range}*"},
    })
    ...
    for cat in CATEGORIES:
        ...
        lines.append(
            f"{CAT_EMOJI[cat]} *{cat}*　`{bar}`　"
            f"{fmt_time(v['minutes'])}　{v['count']}件　_{pct}%_"
        )
    ...
    for date_str in sorted(by_day):
        ...
        day_lines.append(
            f"　*{weekday_ja(date_str)}*　{fmt_time(day_min)}　{len(day_evs)}件"
        )
    blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": "\n".join(day_lines)}})
    blocks.append({"type": "divider"})
    ...
    blocks.append({
        "type": "section",
        "text": {"type": "mrkdwn", "text": "*📋 全会議リスト（承諾済み・複数人）*"},
    })
    ...
    for cat in CATEGORIES:
        ...
        for ev in evs:
            lines.append(
                f"　• {weekday_ja(ev['date'])} {ev['time']}　{ev['title']}　"
                f"_{fmt_time(ev['minutes'])}_"
            )
        blocks.append({
            "type": "section",
...
def post_dm(blocks: list):
    ...
    print(f"Slack DM 投稿完了 → channel: {channel}")
...
def run(week_start: datetime = None, week_end: datetime = None):
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) == 3:
        # 手動実行: python meeting_summary.py 2026-03-23 2026-03-28
